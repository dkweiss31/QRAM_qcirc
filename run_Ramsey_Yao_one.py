...
temp = 0.1
ramsey_experiment_one = RamseyExperiment(omega_tmon, np.array([omega_a, ]), np.array([chiaq, ]), np.array([kappa_a, ]),
                                         temp,
                                         tmon_dim, cavity_dim, 1)
...
ramsey_result = ramsey_experiment_one.ramsey_liouv(thermal_state, delay_times, omega_d, c_ops)
gamma_phi, popt, pcov = ramsey_experiment_one.extract_gammaphi(ramsey_result, delay_times)
