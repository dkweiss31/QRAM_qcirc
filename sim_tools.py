...
class SimulateBosonicOperations():
    def __init__(self, sx, sy, sz, id_cav, sx_cav, sy_cav, sz_cav, chi, tmon_dim, cavity_dim):
        self.sx = sx
        self.sy = sy
        self.sz = sz
        self.sx_cav = sx_cav
    ...
    def cZZU(self, a_op: Qobj, b_op: Qobj, c_ops=None):
        ...
        H = -0.5 * self.chi * self.sz * a_op.dag() * a_op+ 0.5 * g * (a_op.dag() * b_op + a_op * b_op.dag())
    ...
    def id_wrap_ops(self, op: Qobj, idx: int, truncated_dims: list):
    ...
    def construct_basis_states_list(self, Fock_states_spec: list, truncated_dims: list):
        basis_states = []
    ...
    def project_U(U: Qobj, Fock_states_spec: list, truncated_dims: list):
        ...
        for i, state_spec in enumerate(Fock_states_spec):
            basis_list = [basis(truncated_dims[j], state_spec[j])
    ...
    def logical_error_prob(self, U_diss, initial_Fock_states_spec, bad_Fock_states_spec, truncated_dims):
        """
        Parameters
        ----------
        ...
        """
        initial_basis_list = self.construct_basis_states_list(initial_Fock_states_spec, truncated_dims)
        num_initial_states = len(initial_Fock_states_spec)
        bad_basis_list = self.construct_basis_states_list(bad_Fock_states_spec, truncated_dims)
        bad_pop_total = 0.0
    ...
    def gate_failure_prob(self, U_diss, initial_Fock_states_spec, detected_Fock_states_spec, truncated_dims):
        initial_basis_list = self.construct_basis_states_list(initial_Fock_states_spec, truncated_dims)
        num_initial_states = len(initial_Fock_states_spec)
        detected_basis_list = self.construct_basis_states_list(detected_Fock_states_spec, truncated_dims)
    ...
    def truncate_superoperator(self, superop, keep_idxs):
        """
        Parameters
        ----------
        superop
    ...
    def calc_fidel_chi(self, chi_real, chi_ideal):
    ...
    def U_eJP_func(self, a_op:Qobj, b_op:Qobj, params, c_ops=None):
        ...
        U_tmon_y_min = self.R_tmon(-tmon_d_strength, tmon_d_time, direction='Y', c_ops=c_ops)
    ...
    def epsilon_undid(self, U_eJP, U_eJP_ideal, initial_state):
        ...
        return 1 - initial_state.dag() * kraus_list[0].dag() * kraus_list[0] * initial_state
    ...
    def measurement_channel(self, rho, measurement_op):
        if measurement_op.type == 'oper':
            ...
            raise RuntimeError('measurement_op should be either of type "oper" or "super"')
    ...
    # def epsilon_undid_measure(self, U_eJP, U_eJP_ideal, initial_state):
    #     kraus_list = self.kraus_list_measure(to_super(U_eJP_ideal.dag()) * U_eJP, initial_rho)
    #     return 1 - initial_state.dag() * kraus_list[0].dag() * kraus_list[0] * initial_state
    ...
    def fidelity_kraus(self, superop, ideal_op, keep_idxs):
        ...
        return (1.0 / (dim * (dim + 1))) * (np.trace(np.sum([kraus_op.dag() * kraus_op for kraus_op in kraus_m_ops]))
                                            + np.sum([np.abs(np.trace(kraus_op)) ** 2 for kraus_op in kraus_m_ops]))


    def failure_rate(self, U_real, num_qubits=2):
        comp_basis_states, superpos_states = self.state_basis(num_qubits)
        all_init_states = comp_basis_states + superpos_states
        failure_rate = 0.0
        measurement_op = tensor(qeye(self.cavity_dim), qeye(self.cavity_dim), basis(self.tmon_dim, 0) * basis(self.tmon_dim, 0).dag())
        for idx, state in enumerate(all_init_states):
            rho = operator_to_vector(state * state.dag())
            propagated_rho = vector_to_operator(self.apply_channel(rho, U_real, measurement_op=None))
            failure_rate += 1 - np.trace(measurement_op * propagated_rho)
        return failure_rate / len(all_init_states)


    def fidelity_nielsen(self, U_real, U_ideal, Fock_states_spec, truncated_dims,
                        measurement_op=None, ptrace_idxs=None, num_qubits=2):
        dim = 2 ** num_qubits
        op_basis, pauli_keys = self.operator_basis(num_qubits)
        st_contr = 0.0
        for j, op in enumerate(op_basis):
            alpha_state_pair = self.decompose_op_into_state_basis(num_qubits, op, pauli_keys[j])
            for k, (coeff, pauli_rho) in enumerate(alpha_state_pair):
                rho = operator_to_vector(pauli_rho)
                propagated_rho = vector_to_operator(self.apply_channel(rho, U_real, measurement_op=measurement_op))
                if ptrace_idxs is not None:
                    propagated_rho = propagated_rho.ptrace(ptrace_idxs)
                    op = op.ptrace(ptrace_idxs)
                projected_rho = self.project_U(propagated_rho, Fock_states_spec, truncated_dims)
                projected_op = self.project_U(op, Fock_states_spec, truncated_dims)
                #            projected_U_ideal = project_U(U_ideal, Fock_states_spec, truncated_dims)
                #            print(propagated_rho, op, projected_rho)
                st_contr += coeff * np.trace(U_ideal * projected_op.dag()
                                            * U_ideal.dag() * projected_rho)
        return (st_contr + dim ** 2) / (dim ** 2 * (dim + 1))


    def state_basis(self, comp_basis_states, num_qubits):
        superpos_states = []
        for i, comp_bas_state_1 in enumerate(comp_basis_states):
            for j, comp_bas_state_2 in enumerate(comp_basis_states):
                if j > i:
                    superpos_states.append((comp_bas_state_1 + comp_bas_state_2).unit())
                    superpos_states.append((comp_bas_state_1 - comp_bas_state_2).unit())
                    superpos_states.append((comp_bas_state_1 + 1j * comp_bas_state_2).unit())
                    superpos_states.append((comp_bas_state_1 - 1j * comp_bas_state_2).unit())
        return comp_basis_states, superpos_states


    def decompose_op_into_state_basis(self, num_qubits, op, pauli_key):
        comp_basis_states, superpos_basis_states = self.state_basis(num_qubits)
        ...
        alpha_state_pair = [(alpha_coeffs[i], state) for i, state in enumerate(st_rho) if alpha_coeffs[i] != 0]
        return alpha_state_pair


    def operator_basis(self, num_qubits):  # dummy argument for now
        ops = {'I': self.id_cav, 'X': self.sx_cav, 'Y': self.sy_cav, 'Z': self.sz_cav}
        pauli_ops = []
        pauli_keys = []
        for key_a in ops.keys():
            for key_b in ops.keys():
                # start with the tmon in the ground state
                pauli_ops.append(tensor(ops[key_a], ops[key_b], basis(self.tmon_dim, 0) * basis(self.tmon_dim, 0).dag()))
    ...
    def test_cZZU(self):
        ...
        cavity_fock_trunc = 2
        a= self.id_wrap_ops(destroy(cavity_dim), cav_a_idx, truncated_dims)
        b = self.id_wrap_ops(destroy(cavity_dim), cav_b_idx, truncated_dims)
        sz = self.id_wrap_ops(sigmaz(), tmon_idx, truncated_dims)
        chi = 2.0 * np.pi * 0.001
        Fock_states_spec = [(i, j, k)
                            for i in range(cavity_fock_trunc)
                                for j in range(cavity_fock_trunc)
                                for k in range(2)]
        cZZU_projected = self.project_U(self.cZZU(chi, a, b, sz), Fock_states_spec, truncated_dims)
        ideal_cZZU = (-1j * (np.pi / 2) * sz * (a.dag() * a + b.dag() * b)).expm()
        ideal_cZZU_projected = self.project_U(ideal_cZZU, Fock_states_spec, truncated_dims)
