...
class SimulateGUETwoWay(SimulateGUE):
    ...
    def __init__(self, gamma_a_avg: float, gamma_b_avg: float, gamma_c_avg: float, gamma_a_dev: float,
                 ...
                 cavity_dim: int = 2, scale_a: float = 1.017, scale_b: float = 1.018, scale_c: float = 1.017,
                 t_half: float = 600.0, B: float = 0.006, c: float = 2.8284e-5, Gamma_1_cav: float = 0.0,
                 Gamma_phi_cav: float = 0.0, Gamma_1_transfer_nr: float = 0.0, Gamma_phi_transfer: float = 0.0,
                 nth: float = 0.0, nsteps: int = 2000, atol: float = 1e-10, rtol: float = 1e-10, num_cpus: int = 8):
        super().__init__(gamma_b_avg, gamma_c_avg, gamma_b_dev, gamma_c_dev, cav_idx_dict, tran_res_idx_dict,
                         ...
                         Gamma_phi_transfer, nth, nsteps, atol, rtol, num_cpus)
        ...
        self.a1 = id_wrap_ops(destroy(cavity_dim), cav_idx_dict["a1_idx"], self.truncated_dims)
        self.a2 = id_wrap_ops(destroy(cavity_dim), cav_idx_dict["a2_idx"], self.truncated_dims)
        self.b1 = id_wrap_ops(destroy(cavity_dim), cav_idx_dict["b1_idx"], self.truncated_dims)
        self.b2 = id_wrap_ops(destroy(cavity_dim), cav_idx_dict["b2_idx"], self.truncated_dims)
        self.c1 = id_wrap_ops(destroy(cavity_dim), cav_idx_dict["c1_idx"], self.truncated_dims)
        self.c2 = id_wrap_ops(destroy(cavity_dim), cav_idx_dict["c2_idx"], self.truncated_dims)

        self.a1_r = id_wrap_ops(destroy(cavity_dim), tran_res_idx_dict["a1_r_idx"], self.truncated_dims)
        self.a2_r = id_wrap_ops(destroy(cavity_dim), tran_res_idx_dict["a2_r_idx"], self.truncated_dims)
        self.b1_r = id_wrap_ops(destroy(cavity_dim), tran_res_idx_dict["b1_r_idx"], self.truncated_dims)
        self.b2_r = id_wrap_ops(destroy(cavity_dim), tran_res_idx_dict["b2_r_idx"], self.truncated_dims)
        self.c1_r = id_wrap_ops(destroy(cavity_dim), tran_res_idx_dict["c1_r_idx"], self.truncated_dims)
        self.c2_r = id_wrap_ops(destroy(cavity_dim), tran_res_idx_dict["c2_r_idx"], self.truncated_dims)
...
class SimulateGUEDR(SimulateGUE, DualRailMixin):
    ...
    def measurement_op_DR(self, idx_0, idx_1):
        ...
        if self.additional_label:
            ...
            vac_proj = tensor(*[basis(dim, 0) * basis(dim, 0).dag() for dim in [dim_0, dim_1, 2]])
            return (
            ...
            vac_proj = tensor(*[basis(dim, 0) * basis(dim, 0).dag() for dim in [dim_0, dim_1]])
            return (
                tensor(right_state_proj, vac_proj)
                + tensor(vac_proj, right_state_proj)
            )
...
class SimulateGUETwoWayDR(SimulateGUETwoWay, DualRailMixin):
    def __init__(self, gamma_a_avg: float, gamma_b_avg: float, gamma_c_avg: float, gamma_a_dev: float,
                 gamma_b_dev: float, gamma_c_dev: float, cav_idx_dict: dict, tran_res_idx_dict: dict):
        super().__init__(gamma_a_avg, gamma_b_avg, gamma_c_avg, gamma_a_dev, gamma_b_dev, gamma_c_dev, cav_idx_dict,
                         tran_res_idx_dict)
