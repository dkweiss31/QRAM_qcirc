...
delay_times = np.linspace(0.0, 25000, 301)
thermal_time = 200.0
...
full_dims = [cav_dim, tmon_dim]
full_dim = cav_dim * tmon_dim
truncated_dims = [cav_truncated_dim, tmon_truncated_dim]
dim_after_diag = cav_truncated_dim * tmon_truncated_dim
...
a, q = dq.destroy(12, 12)

phi_a = phi_cav(alpha, EJ, chi_cavstmon, idx=0)
phi_q = phi_q_func(alpha, EJ)

bare_labels = np.array(list(np.ndindex(*full_dims)))
_H0 = omega_a * dq.dag(a) @ a + omega_tmon * dq.dag(q) @ q
_H0 += -EJ * dq.cosm(phi_q * (q + dq.dag(q)) + phi_a * (a + dq.dag(a)))
_H0 += -0.5 * EJ * phi_q ** 2 * dq.powm(q + dq.dag(q), 2) - 0.5 * EJ * phi_a ** 2 * dq.powm(a + dq.dag(a), 2)
_H0 += -EJ * phi_a * phi_q * (q + dq.dag(q)) @ (a + dq.dag(a))
...
for bare_label in bare_labels:
    init_ket = dq.tensor(
        ...
        dq.basis(tmon_dim, bare_label[1])
    )
...
chi = (evals[bare_to_dressed_labels[(1, 1)]]
       - evals[bare_to_dressed_labels[(1, 0)]]
       - evals[bare_to_dressed_labels[(0, 1)]]
       + evals[bare_to_dressed_labels[(0, 0)]]
       )
print(chi/2/np.pi)
omega_q_tilde = evals[bare_to_dressed_labels[(0, 1)]] - evals[bare_to_dressed_labels[(0, 0)]]
omega_a_tilde = evals[bare_to_dressed_labels[(1, 0)]] - evals[bare_to_dressed_labels[(0, 0)]]
omega_d_cav = omega_a_tilde

# define rotating frame to be drive of cavity and frequency of qubit
rot_frame_diag = jnp.squeeze(jnp.asarray([omega_d_cav * bare_1 + omega_q_tilde * bare_2
                              for (bare_1, bare_2) in truncated_bare_labels]))
n_op_a = dq.tensor(dq.number(cav_truncated_dim), dq.eye(tmon_truncated_dim))
...
a_trunc_bare = dq.tensor(dq.destroy(cav_truncated_dim), dq.eye(tmon_truncated_dim))
...
e_ops = [n_op_a, dq.dag(q_tilde_trunc) @ q_tilde_trunc]
# TODO nonzero temp
c_ops = [dq.timecallable(lambda t: np.sqrt(kappa_a) * a_trunc_bare * jnp.exp(1j * rot_frame_drive * t)), ]
options = dq.Options(
...
if exp_type == "ramsey":
    ...
    m_op = dq.tensor(dq.eye(cav_truncated_dim), _psi0 @ dq.dag(_psi0))
    p0 = (1 * 10 ** 5, 5.69661179e-04, 0.5, 0.5, 0.1)
elif exp_type == "T1":
    psi0 = dq.basis(dim_after_diag, 1)
    m_op = dq.tensor(dq.eye(cav_truncated_dim), dq.basis(tmon_truncated_dim, 1) @ dq.dag(dq.basis(tmon_truncated_dim, 1)))
    p0 = (4 * 10 ** 3, 1.0, 0.0,)
else:
...
def mesolve_for_final_state(H, init_dm, t):
    ...
    nbar = np.mean(result.expects[0])
    print("expectation value of n_1: ", np.max(result.expects[0]))
    print("analytical coherent kappa = ", gamma_coherent(nbar, kappa_a, chi, 0.0)/(2.0*np.pi) * 10**6)
...
def main_ramsey(
    ...
    write_to_h5(filepath, {"ramsey_result": ramsey_result}, {})
    if exp_type == "ramsey":
...
def extract_gammaphi(
    ...
    print("stdev of gamma_2 = ", (1. / pcov_T2[0, 0]) * 10**6 / (2 * np.pi), "kHz")
